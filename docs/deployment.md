...
- 自定义域名和 DNS 记录在 Cloudflare 控制台维护，不由 GitHub Actions 修改
...
`CLOUDFLARE_API_TOKEN` 需要至少具备 Cloudflare Pages Edit 权限。若需要由 workflow 首次创建 Pages project，该 token 也必须允许创建 Pages 项目。GitHub Actions 不需要 DNS 写入权限。
