...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  const version = await env.DB.prepare('SELECT id FROM content_versions WHERE id = ? AND item_id = ? AND status = ?')
    ...
    .first<{ id: string }>()
  if (!version) return errorJson(400, 'validation_error', '只能发布已批准版本。')
  ...
  const insertPublish = env.DB.prepare('INSERT INTO publish_events (item_id, from_version_id, to_version_id, actor_id, note) VALUES (?, ?, ?, ?, ?)')
    .bind(itemId, item?.published_version_id || null, body.versionId, user.id, body.note || '')
